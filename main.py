...
def create_user_friendly_chat_chain():
    ...
    **중요: 참고자료에서 [판례-숫자] 형태로 표시된 판례가 있다면 반드시 해당 판례번호를 정확히 인용하세요.**
    
    1. [첫 번째 판례 내용 설명]
    2. [두 번째 판례 내용 설명]
    
    **[참고: 판례-186226, 판례-224875]** (실제 참고자료에 나타난 판례번호 사용)
    ...
    → 출처 표기: **[참고: 법령해석례-ID번호]** (실제 ID가 있다면)

    생활법령정보 '백문백답' 중 유사 사례가 있다면 사용자 질문에 맞게 설명하며 연결해줍니다.  
    → 출처 표기: **[참고: 백문백답-ID번호]** (실제 ID가 있다면)
    ...
    ### 📌 중요 지침
    - 참고자료에 나타난 **판례번호, 해석례번호, 백문백답번호를 정확히 인용**하세요.  
